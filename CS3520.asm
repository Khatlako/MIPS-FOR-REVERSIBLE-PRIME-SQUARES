...
.text
	...
	#THE FUNCTION FINDS THE SQUARE ROOT OF NUMBER
	#BY CALLING OTHER FUNCTIONS AND USING THEIR ATTRIBUTES
	sqrt:
	move $20, $zero	
	...
	#sqrofaprime fcn
	# THIS FUNCTION SQUARES THE PRIME NUMBER
	#MULTIPLIES A PRIME NUMBER BY ITSELF
	sqrofaprime:
		add $v1, $zero, $t1
		jal sqrt
		jal Isprime
  	...
  	# THIS FUNCTION CHECKS WHETHER THE REVERSED IS A SQUARE NUMBER OR NOT
  	#INTIALISES $VI WITH THE VALUE OF $T1
  	#REVERSES THE VALUE OF $VI 
  	#FINDS ITS  SQUARE ROOT
  	#CHECKS IF THE ROOT IS A PRIME NUMBER 
  	revissqrofaprime:
  		#move $t1 to $v1 
  		add $v1, $zero, $t1
  		#reverse v1
